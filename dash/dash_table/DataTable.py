...
class DataTable(Component):
    ...
    - data (list of dicts with strings as keys and values of type string | number | boolean; optional):
        ...
        display the row ID, otherwise it is  just used to reference the
        row for selections, filtering, etc.  Example:  [
    ...
    - columns (list of dicts; optional):
        ...
        - editable (boolean; optional):
            ...
            `editable` flag.  These flags determine whether the contents
            of the table  are editable or not.  If the column-level
            `editable` flag is set it overrides  the table-level
            `editable` flag for that column.

        - filter_options (dict; optional):
            There are two `filter_options` props in the table.  This is
            the column-level filter_options prop and there is  also the
        ...
        - format (dict; optional):
            ...
            - locale (dict; optional):
                ...
                - symbol (list of strings; optional):
                    ...
                    representing the   prefix and suffix symbols.
                    Typically used for currency, and implemented using
                    d3's   currency format, but you can use this for other
        ...
        - id (string; required):
            The `id` of the column.  The column `id` is used to match
            cells in data with particular columns.  The `id` is not
        ...
        - selectable (a value equal to: 'first', 'last' | boolean | list of booleans; optional):
            ...
            `first` it will only  appear on the first header row. These
            are respectively shortcut equivalents to  `[False, ..., False,
        ...
        - type (a value equal to: 'any', 'numeric', 'text', 'datetime'; optional):
            ...
            allows for data  validation and coercion.  'numeric':
            represents both floats and ints.  'text': represents a string.
            'datetime': a string representing a date or date-time, in the
            form:    'YYYY-MM-DD HH:MM:SS.ssssss' or some truncation
            thereof. Years must    have 4 digits, unless you use
            `validation.allow_YY: True`. Also    accepts 'T' or 't'
            between date and time, and allows timezone info    at the end.
            To convert these strings to Python `datetime` objects,    use
            `dateutil.parser.isoparse`. In R use `parse_iso_8601` from the
            `parsedate` library.    WARNING: these parsers do not work
            with 2-digit years, if you use    `validation.allow_YY: True`
            and do not coerce to 4-digit years.    And parsers that do
            work with 2-digit years may make a different    guess about
            the century than we make on the front end.  'any': represents
            any type of data.  Defaults to 'any' if undefined.
        ...
        - validation (dict; optional):
            ...
            - allow_YY (boolean; optional):
                ...
                (default: False).    If True, we interpret years as
                ranging from now-70 to now+29 - in 2019    this is 1949 to
                2048 but in 2020 it will be different. If used with
                `action: 'coerce'`, will convert user input to a 4-digit
                year.
    ...
    - fixed_rows (dict; default {
        ...
        header and filter rows (see `filter_action`) are  fixed.
        Additional data rows can be fixed by assigning  a number to
        `data`.  Note that fixing rows introduces some changes to the
        underlying markup of the table and may impact the  way that your
        columns are rendered or sized.  View the documentation examples to
        learn more.
    ...
    - column_selectable (a value equal to: 'single', 'multi', false; default False):
        ...
        merged columns via the radio button that will appear in the
        header rows.  If `multi`, then the user can select multiple
        columns or groups  of merged columns via the checkbox that will
        appear in the header  rows.  If False, then the user will not be
        able to select columns and no  input will appear in the header
        rows.  When a column is selected, its id will be contained in
        `selected_columns`  and `derived_viewport_selected_columns`.
    ...
    - row_selectable (a value equal to: 'single', 'multi', false; default False):
        ...
        user can select multiple rows  via a checkbox that will appear
        next to each row.  If False, then the user will not be able to
        select rows  and no additional UI elements will appear.  When a
        row is selected, its index will be contained  in `selected_rows`.
    ...
    - selected_cells (list of dicts; optional):
        ...
        Multiple cells can be selected by holding down shift and  clicking
        on a different cell or holding down shift and navigating  with the
        arrow keys.
    ...
    - selected_rows (list of numbers; optional):
        `selected_rows` contains the indices of rows that  are selected
        via the UI elements that appear when  `row_selectable` is
        `'single'` or `'multi'`.
    ...
    - selected_row_ids (list of string | numbers; optional):
        ...
        the UI elements that appear when  `row_selectable` is `'single'`
        or `'multi'`.

    - start_cell (dict; optional):
        When selecting multiple cells  (via clicking on a cell and then
        shift-clicking on another cell),  `start_cell` represents the
        [row, column] coordinates of the cell  in one of the corners of
        the region.  `end_cell` represents the coordinates of the other
        corner.
    ...
    - merge_duplicate_headers (boolean; optional):
        ...
        names  will be merged into a single cell.  This will be applied
        for single column headers and multi-column  headers.
    ...
    - data_timestamp (number; optional):
        ...
        `n_clicks_timestamp` in `dash_html_components`)  to determine
        which property has changed within a callback.
    ...
    - page_action (a value equal to: 'custom', 'native', 'none'; default 'native'):
        ...
        a subset of them.  With pagination, we can either page through
        data that exists  in the table (e.g. page through `10,000` rows in
        ...
        table up-front, paging logic is  handled by the table;
        `'custom'`: data is passed to the table one page at a time, paging
        logic  is handled via callbacks;  `'none'`: disables paging,
        render all of the data at once.
    ...
    - filter_action (dict; default 'none'):
        ...
        responsibility of the developer to program the filtering  through
        a callback (where `filter_query` or
        `derived_filter_query_structure` would be the input  and `data`
        would be the output).
    ...
    - filter_options (dict; optional):
        ...
        table-level filter_options prop and there is  also the
        column-level `filter_options` prop.  These props determine whether
        the applicable filter relational  operators will default to
        `sensitive` or `insensitive` comparison.  If the column-level
    ...
    - sort_action (a value equal to: 'custom', 'native', 'none'; default 'none'):
        ...
        displayed.  If `'native'`, then the sorting UI is displayed and
        the sorting  logic is handled by the table. That is, it is
        performed on the data  that exists in the `data` property.  If
        `'custom'`, the the sorting UI is displayed but it is the
        responsibility of the developer to program the sorting  through a
        callback (where `sort_by` would be the input and `data`  would be
        the output).  Clicking on the sort arrows will update the
        `sort_by` property.
    ...
    - sort_mode (a value equal to: 'single', 'multi'; default 'single'):
        ...
        column.  NOTE - With multi-column sort, it's currently  not
        possible to determine the order in which  the columns were sorted
        through the UI.  See
    ...
    - sort_by (list of dicts; optional):
        `sort_by` describes the current state  of the sorting UI.  That
        is, if the user clicked on the sort arrow  of a column, then this
        property will be updated  with the column ID and the direction
        (`asc` or `desc`) of the sort.  For multi-column sorting, this
        will be a list of  sorting parameters, in the order in which they
        were  clicked.
    ...
    - sort_as_null (list of string | number | booleans; optional):
        ...
        `None`  (i.e. ignored and always displayed last) when sorting.
        This value will be used by columns without `sort_as_None`.
        Defaults to `[]`.
    ...
    - dropdown (dict; optional):
        ...
        whether the value can be deleted.  The `options` property refers
        to the `options` of the dropdown.
    ...
    - tooltip (dict; optional):
        ...
        - value (string; required):
            ...
            is required. Alternatively, the value of the  property can
            also be  a plain string. The `text` syntax will be used in
            that case.
    ...
    - tooltip_conditional (list of dicts; optional):
        ...
        have  different tooltips in the same column based on the value  of
        a certain data property.  Priority is from first to last defined
        ...
        - if (dict; required):
            ...
            in order for the associated tooltip configuration to be  used.
            If multiple conditions are defined, all conditions  must be
            met for the tooltip to be used by a cell.
    ...
    - tooltip_header (dict; optional):
        ...
        column and optionally each header row.  Example to show long
        column names in a tooltip: {i: i for i in df.columns}.  Example to
        show different column names in a tooltip: {'Rep': 'Republican',
        'Dem': 'Democrat'}.  If the table has multiple rows of headers,
        then use a list as the value of the  `tooltip_header` items.
    ...
    - tooltip_duration (number; default 2000):
        ...
        hovering a cell. If  set to `None`, the tooltip will not
        disappear.  Defaults to 2000.
    ...
    - locale_format (dict; optional):
        ...
        - symbol (list of strings; optional):
            ...
            implemented using d3's    currency format, but you can use
            this for other symbols such as measurement units.
    ...
    - fill_width (boolean; default True):
        ...
        behaviors:  True: The table container's width will grow to fill
        the available space;  False: The table container's width will
        equal the width of its content.
    ...
    - markdown_options (dict; default {
        ...
        - link_target (string | a value equal to: '_blank', '_parent', '_self', '_top'; optional):
            ...
            link in a  new tab, _parent opens the link in the parent
            frame, _self opens the link in the  current tab, and _top
            opens the link in the top frame) or a string.
    ...
    - derived_filter_query_structure (dict; optional):
        ...
        (any):    'expression, value': passed value,    'expression,
        field': the field/prop name.  block (nested query structure;
        optional).  left (nested query structure; optional).  right
        (nested query structure; optional).  If the query is invalid or
        empty, the `derived_filter_query_structure` will  be `None`.
    ...
    - persistence (boolean | string | number; optional):
        ...
        is truthy and  hasn't changed from its previous value, any
    ...
    - persisted_props (list of a value equal to: 'columns.name', 'data', 'filter_query', 'hidden_columns', 'page_current', 'selected_columns', 'selected_rows', 'sort_by's; default [
        'columns.name',
        'filter_query',
        'hidden_columns',
        'page_current',
